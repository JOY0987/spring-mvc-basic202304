...
<script>
    ...
    function makeReplyRegisterClickEvent() {
        ...
        $regBtn.onclick = e => {
            ...
            // 내가 댓글창에 입력한 값이 나오는지 확인해보며 코드짜기
            // console.log($rt);
            // console.log($rw);

            // 서버로 보내기 전, 클라이언트 입력값 검증
            // validation 검증 함수를 따로 만드는 것도 좋다.
            if ($rt.value.trim() === '') {
                alert('댓글 내용은 필수입니다!');
                return;
            }
            ...
            if ($rw.value.trim().length < 2 || $rw.value.trim().length > 8) { // 문자열의 길이 비교
            ...
            fetch(URL, requestInfo)
                .then(res => {
                    if (res.status === 200) {
                        ...
                        getReplyList(lastPageNo); // 댓글이 마지막에 추가되므로, 마지막 페이지 번호가 들어와야 함
                    } else {
                        alert('댓글 등록에 실패함!');
                    }
                })
    ...
    window.onload = function () {
        $heartIcon = document.querySelector('.fas');
      if (${b.likeCount} === 0) {
